...
[MAC_BASE] = LAYOUT_tkl_ansi(
     ...
     ESC_TILDE,KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,   KC_BSPC, KC_INS,    KC_HOME,  KC_PGUP,
    ...
    switch (keycode) {
        case ESC_TILDE:
            if (record->event.pressed) {
                ...
                // FN 레이어가 활성화된 경우 (MAC_FN 또는 WIN_FN)
                if (IS_LAYER_ON(MAC_FN) || IS_LAYER_ON(WIN_FN)) {
                    tap_code(KC_GRV);  // ` 입력
                }
                // Shift가 눌린 경우
                else if (mods & MOD_MASK_SHIFT) {
                    tap_code16(S(KC_GRV));  // ~ 입력
                }
                // 일반 누름
                else {
                    tap_code(KC_ESC);  // ESC 입력
                }
            }
            return false;
    }
